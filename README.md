...
Ponieważ moduły perl zarządzane są z poziomu menandżera paczek `pacman` trzeba posłużyć się metodą ręczną (lub jeśli ktoś ma ochotę, stworzyć paczkę w AUR).

Wklej moduł do poniższej lokalizacji
```bash
$ sudo cp ObligacjeSP.pm /usr/share/perl5/vendor_perl/Finance/Quote/ObligacjeSP.pm`
```
Zedytuj zgodnie z powyższym plik `Quote.pm` w poniższej lokalizacji

```bash
$ sudo vim /usr/share/perl5/vendor_perl/Finance/Quote.pm`
```
